...
def main(params):
  ...
  for i,img in enumerate(imgs):
    ...
    # write to pkl
    np.save(os.path.join(dir_fc, str(img['dataid'])), tmp_fc.data.cpu().float().numpy())
    np.savez_compressed(os.path.join(dir_att, str(img['dataid'])), feat=tmp_att.data.cpu().float().numpy())
